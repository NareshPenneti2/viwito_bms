...
export class SigninComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    this.signinForm = new UntypedFormGroup({
      username: new UntypedFormControl('naresh.penneti@gmail.com', Validators.required),
      password: new UntypedFormControl('abc123@@@', Validators.required),
  ...
  signin() {
    ...
    // this.authService.signIn(signinData.username, signinData.password)
    //  .then(response => {
    //   this.router.navigateByUrl(this.jwtAuth.return);
    // }, err => {
    //   this.submitButton.disabled = false;
